<template>
  <Layout>
    <h1 class="text-color">タグ: {{$context.tag}}</h1>
...
export default {
  ...
  methods: {
    ...
    paging: function(num) {
      let pathToGo = "/list-tag/" + this.$context.tag + "/" + num;
...
<page-query>
query Blog ( $tag: String!, $count: Int ) {
  allContentfulBlog( perPage: 9, sortBy: "createdate", order: DESC, page: $count, filter: {
    tags: {
      contains: [$tag]
    }
  }) {
